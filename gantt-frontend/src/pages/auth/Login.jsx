...
const Login = () => {
    ...
    const [isConfirmPasswordCorrect, setIsConfirmPasswordCorrect] = useState(false);
    ...
    const buttonText = isLoggingIn ? "Войти" : isSigningIn ? "Зарегистрироваться" : "Продолжить";
    ...
    const isButtonEnabled = isEmailCorrect &&
        ((!isLoggingIn && !isSigningIn) ||
        (isLoggingIn && password.trim() !== "") ||
        (isSigningIn && username.trim() !== "" && password.trim() !== "" && isConfirmPasswordCorrect));
    ...
    const getErrorMessage = async (response) => {
        ...
        return data?.message || data?.detail || data?.error || `Ошибка ${response.status}`;
    }
    ...
    const confirmPassword = (passwordStr = password, confirmPassStr = confirmPasswordText) => {
    ...
    const handleRegistration = async () => {
        try {
            ...
            const response = await fetch("/api/register?email=" + encodeURIComponent(email) +
                "&nickname=" + encodeURIComponent(username) + "&password=" + encodeURIComponent(password), {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                // body: JSON.stringify({
                //     email: email.toLowerCase(),
