...
def get_subscription_by_id(db: orm.Session, subscription_id: int):
    ...
    return (
        db.query(push.PushSubscription)
        .filter(push.PushSubscription.id == subscription_id)
        .first()
    )
...
def get_subscriptions_by_user(db: orm.Session, user_id: int):
    ...
    return (
        db.query(push.PushSubscription)
        .filter(push.PushSubscription.user_id == user_id)
        .all()
    )


def create_subscription(
    db: orm.Session, user_id: int, endpoint: str, p256dh: str, auth: str
):
