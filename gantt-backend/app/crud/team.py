...
def delete_team_with_dependencies(data_base: orm.Session, team_id: int):
    ...
    projects = (
        data_base.query(project_model.Project)
    ...
    data_base.query(tag_model.Tag).filter(tag_model.Tag.team_id == team_id).delete(
        synchronize_session=False
    )

    team_users = (
        data_base.query(team.UserTeam).filter(team.UserTeam.team_id == team_id).all()
    )
    ...
    data_base.query(team.UserTeam).filter(team.UserTeam.team_id == team_id).delete(
        synchronize_session=False
    )
