...
def create_custom_field(
    db: Session, team_id: int, field: custom_field_schema.CustomFieldBase
):
...
def get_custom_fields_by_team(db: Session, team_id: int):
    return (
        db.query(custom_field_model.CustomField)
        .filter(custom_field_model.CustomField.team_id == team_id)
        .all()
    )


def update_custom_field(
    db: Session, field_id: int, field_update: custom_field_schema.CustomFieldBase
):
    db_field = (
        db.query(custom_field_model.CustomField)
        .filter(custom_field_model.CustomField.id == field_id)
        .first()
    )
    if db_field:
...
def set_task_custom_field_value(
    db: Session, task_id: int, field_value: custom_field_schema.TaskCustomFieldValueBase
):
    db_value = (
        db.query(custom_field_model.TaskCustomFieldValue)
        .filter_by(task_id=task_id, custom_field_id=field_value.custom_field_id)
        .first()
    )
    ...
    if db_value:
        ...
        db_value = custom_field_model.TaskCustomFieldValue(
            task_id=task_id, **field_value.model_dump()
        )
        db.add(db_value)
