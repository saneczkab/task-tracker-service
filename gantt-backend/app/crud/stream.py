...
from app.models import custom_field as cf_model, meta as meta_model, tag as tag_model
...
def get_streams_by_project_id(data_base: orm.Session, proj_id: int):
    ...
    return (
        data_base.query(stream.Stream)
        .filter(stream.Stream.project_id == proj_id)
        .order_by(stream.Stream.position)
        .all()
    )
...
def get_stream_by_name_and_proj_id(data_base: orm.Session, name: str, proj_id: int):
    return (
        data_base.query(stream.Stream)
        .filter(stream.Stream.project_id == proj_id, stream.Stream.name == name)
        .first()
    )


def create_new_stream(
    data_base: orm.Session, proj_id: int, stream_data: stream_schemas.StreamCreate
):
    new_stream = stream.Stream(
        name=stream_data.name, project_id=proj_id, position=stream_data.position
    )
...
def update_stream(
    ...
    if (
        stream_update_data.name is not None
        and stream_update_data.name != stream_obj.name
    ):
        existing_stream = (
            data_base.query(stream.Stream)
            .filter(
                stream.Stream.project_id == stream_obj.project_id,
                stream.Stream.name == stream_update_data.name,
                stream.Stream.id != stream_obj.id,
            )
            .first()
        )
        if existing_stream:
            raise exception.ConflictError(
                "Стрим с таким названием уже существует в проекте"
            )
...
def delete_stream(data_base: orm.Session, stream_id: int):
    ...
    all_tasks = data_base.query(task.Task).filter(
