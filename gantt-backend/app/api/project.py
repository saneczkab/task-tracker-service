...
def update_project(
    proj_id: int,
    update_data: project_schemas.ProjectUpdate,
    ...
    try:
        return project_service.update_project_service(
            data_base, proj_id, current_user.id, update_data
        )
...
@router.get(
    "/api/project/{project_id}/tasks", response_model=list[task_schemas.TaskResponse]
)
def get_project_tasks(
    ...
    try:
        return task_service.get_project_tasks_service(
            data_base, project_id, current_user.id
        )
    except exception.NotFoundError as e:
        raise fastapi.HTTPException(404, str(e))
    except exception.ForbiddenError as e:
...
@router.get(
    "/api/project/{proj_id}/streams",
    response_model=list[stream_schemas.StreamResponse],
...
def get_project_streams(
    ...
    try:
        streams = stream_service.get_project_streams_service(
            data_base, proj_id, current_user.id
        )
        return streams
    except exception.NotFoundError as e:
...
@router.post(
    "/api/project/{proj_id}/stream/new",
...
def create_stream(
    ...
    try:
        new_stream = stream_service.create_stream_service(
            data_base, proj_id, stream_data, current_user.id
        )
        return new_stream
    except exception.NotFoundError as e:
        raise fastapi.HTTPException(status_code=404, detail=str(e))
...
def reorder_projects(
    ...
    first_project = project_crud.get_project_by_id(
        data_base, reorder_data.project_ids[0]
    )
    if not first_project:
        raise fastapi.HTTPException(404, "Проект не найден")

    permissions.check_team_access(
        data_base, first_project.team_id, current_user.id, need_lead=True
    )
