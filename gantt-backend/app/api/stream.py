...
def get_stream(
    ...
    try:
        stream = stream_service.get_stream_service(
            data_base, stream_id, current_user.id
        )
        return stream
...
def update_stream(
    ...
    try:
        updated_stream = stream_service.update_stream_service(
            data_base, stream_id, stream_update_data, current_user.id
        )
...
@router.delete(
    "/api/stream/{stream_id}", status_code=fastapi.status.HTTP_204_NO_CONTENT
)
def delete_stream(
    stream_id: int,
    current_user: user_models.User = fastapi.Depends(auth.get_current_user),
...
@router.get(
    "/api/stream/{stream_id}/tasks", response_model=list[task_schemas.TaskResponse]
)
def get_stream_tasks(
    ...
    try:
        return task_service.get_stream_tasks_service(
            data_base, stream_id, current_user.id
        )
    except exception.NotFoundError as e:
...
@router.get(
    "/api/stream/{stream_id}/goals", response_model=list[goal_schemas.GoalResponse]
)
def get_goals(
    stream_id: int,
    current_user=fastapi.Depends(auth.get_current_user),
    ...
    try:
        return goal_service.get_stream_goals_service(
            data_base, stream_id, current_user.id
        )
    except exception.NotFoundError as e:
        raise fastapi.HTTPException(status_code=404, detail=str(e))
...
def create_task(
    ...
    try:
        return task_service.create_task_service(
            data_base, stream_id, current_user.id, task_data
        )
    except exception.NotFoundError as e:
        raise fastapi.HTTPException(404, str(e))
    except exception.ForbiddenError as e:
...
@router.post(
    ...
    response_model=goal_schemas.GoalResponse,
    status_code=201,
)
def create_goal(
    stream_id: int,
    ...
    try:
        return goal_service.create_goal_service(
            data_base, stream_id, current_user.id, goal_data
        )
    except exception.NotFoundError as e:
        raise fastapi.HTTPException(status_code=404, detail=str(e))
    except exception.ForbiddenError as e:
