...
def get_all_tasks_service(data_base: orm.Session, user_id: int):
    user_teams = (
        data_base.query(team.UserTeam).filter(team.UserTeam.user_id == user_id).all()
    )
    ...
    for user_team in user_teams:
        team_projects = (
            ...
            .filter(project.Project.team_id == user_team.team_id)
            .all()
        )
        ...
        for project_obj in team_projects:
            for stream_obj in project_obj.streams:
                stream_tasks = (
                    data_base.query(task.Task)
                    .filter(task.Task.stream_id == stream_obj.id)
                    .all()
                )
                ...
                for task_obj in stream_tasks:
                    ...
                    task_obj.team_name = (
                        project_obj.team.name if project_obj.team else None
                    )
...
def create_task_service(
    ...
    if task_data.position is None:
        last_pos = (
            data_base.query(task.Task)
            .filter(task.Task.stream_id == stream_id)
            .order_by(task.Task.position.desc())
            .first()
        )
    ...
    if task_data.kanban_position is None:
        last_kanban_pos = (
            data_base.query(task.Task)
            .filter(task.Task.stream_id == stream_id)
            .order_by(task.Task.kanban_position.desc())
            .first()
        )
        task_data.kanban_position = (
            (last_kanban_pos.kanban_position + 1) if last_kanban_pos else 1
        )
    ...
    if task_data.custom_fields:
        for field_value in task_data.custom_fields:
            custom_field_crud.set_task_custom_field_value(
                data_base, new_task.id, field_value
            )
...
def update_task_service(
    ...
    if task_update_data.assignee_email:
        ...
        old_user_task = (
            ...
            .filter(meta.UserTask.task_id == task_id)
            .first()
        )
        if old_user_task:
    ...
    if task_update_data.custom_fields is not None:
        ...
        for field_value in task_update_data.custom_fields:
            custom_field_crud.set_task_custom_field_value(
                data_base, task_id, field_value
            )

    task_update_data_filtered = task_update_data.model_dump(
...
def delete_task_relation_service(db: orm.Session, relation_id: int, user_id: int):
    relation = (
        db.query(task.TaskRelation).filter(task.TaskRelation.id == relation_id).first()
    )

    if not relation:
        raise exception.NotFoundError("Связь не найдена")

    _, _, _, _ = permissions.check_task_access(
        db, relation.task_id_1, user_id, need_lead=True
    )
...
def create_task_relation_service(
    data_base: orm.Session, task_id_1: int, task_id_2: int, connection_id: int
):
    ...
    connection_type = (
        data_base.query(meta.ConnectionType)
        .filter(meta.ConnectionType.id == connection_id)
        .first()
    )
    if not connection_type:
        raise exception.NotFoundError("Тип связи не найден")

    return task_crud.create_task_relation(
        data_base, task_id_1, task_id_2, connection_id
    )
...
def delete_task_custom_field_service(
    data_base: orm.Session, task_id: int, custom_field_id: int, user_id: int
):
    """Удалить значение кастомного поля для задачи."""
    task_obj, _, _, team_obj = permissions.check_task_access(
        data_base, task_id, user_id, need_lead=True
    )

    custom_field_obj = (
        data_base.query(custom_field.TaskCustomFieldValue)
        .filter_by(task_id=task_id, custom_field_id=custom_field_id)
        .first()
    )
    ...
    return custom_field_crud.delete_task_custom_field_value(
        data_base, task_id, custom_field_id
    )
