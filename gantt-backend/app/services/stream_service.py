...
def create_stream_service(
    ...
    existing = stream_crud.get_stream_by_name_and_proj_id(
        data_base, stream_data.name, project_id
    )
    if existing:
        raise exception.ConflictError(
            "В данном проекте уже есть стрим с таким названием"
        )
    ...
    if stream_data.position is None:
        last_stream = (
            ...
            .order_by(stream_model.Stream.position.desc())
            .first()
        )
...
def update_stream_service(
    data_base: Session,
    stream_id: int,
    update_data: stream_schemas.StreamUpdate,
    user_id: int,
):
    """Обновить стрим"""
    stream, _, user_team = permissions.check_stream_access(
        data_base, stream_id, user_id
    )
