...
def create_goal_service(
    data_base: orm.Session, stream_id: int, user_id: int, goal_data
):
    ...
    if goal_data.position is None:
        last = (
            data_base.query(goal.Goal)
            .filter_by(stream_id=stream_id)
            .order_by(goal.Goal.position.desc())
            .first()
        )
...
def update_goal_service(
    data_base: orm.Session, goal_id: int, user_id: int, goal_update_data
):
    goal_obj = goal_crud.get_goal_by_id(data_base, goal_id)
    if not goal_obj:
        raise exception.NotFoundError("Цель не найдена")

    permissions.check_stream_access(
        data_base, goal_obj.stream_id, user_id, need_lead=True
    )

    if goal_update_data.name:
        if goal_update_data.name != goal_obj.name:
            existing = goal_crud.get_goal_by_name_in_stream(
                data_base, goal_obj.stream_id, goal_update_data.name, exclude_id=goal_id
            )
            if existing:
                raise exception.ConflictError(
                    "Цель с таким названием уже существует в стриме"
                )
