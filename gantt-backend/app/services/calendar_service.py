...
def export_calendar_service(
    data_base: orm.Session, user_id: int, export_options: calendar.CalendarExport
) -> str:
    tasks = []
    if export_options.target == "all":
        ...
        tasks = get_tasks_by_team_id(
            data_base,
            export_options.target_id,
            user_id if export_options.scope == "my" else None,
        )
    elif export_options.target == "stream":
        tasks = get_tasks_by_stream_id(
            data_base,
            export_options.target_id,
            user_id if export_options.scope == "my" else None,
        )
    elif export_options.target == "teams":
        tasks = get_tasks_by_team_ids(
            data_base,
            export_options.target_ids,
            user_id if export_options.scope == "my" else None,
        )
    elif export_options.target == "projects":
        tasks = get_tasks_by_project_ids(
            data_base,
            export_options.target_ids,
            user_id if export_options.scope == "my" else None,
        )
    elif export_options.target == "streams":
        tasks = get_tasks_by_stream_ids(
            data_base,
            export_options.target_ids,
            user_id if export_options.scope == "my" else None,
        )
